...
if(isset($_POST['decrypt'])){
    $originpass = $_POST['password']; //şifrelenmiş metin
    $originkey = $_POST['key']; //key değerimiz
    
    $ch = str_split($originpass); //metni parçalıyoruz
    $key = str_split($originkey); //keyi parçalıyoruz
    
    $b = range('a', 'z'); //a'dan z'ye kadar olan bir dizi
    array_unshift($b , "z"); //$b dizisinin indisini 1'den başlatmak için boş eleman ekleyip 1.indisi siliyoruz
    ...
    for ($i = 0; $i < 26; $i++) { //26 kere dön
        if (!array_key_exists($i, $ch)) { //eğer $i değeri sifre uzunluğundan büyükse döngüden çık. (key harf sayısı kadar dönüyor döngü)
            ...
            $passvalue = (array_search($ch[$i], $b)); // harf dizimizde şifrelenmiş metnin değerini arıyoruz. dönen yanıt karşılık gelen value değerinin sayısı
            $item = $passvalue - $keyvalue;
            if($item <= 0){
                $item = 26 - abs($item);
            }
            $equal[] = (int)(fmod(($item), 26)-1); //şifreden dönen sayı ile keyden dönen sayıyı toplayıp 26'ya göre modunu alıyoruz ve integer a çevirip $equal dizisine ekliyoruz.
            print_r($item);
            echo "<br>";
    ...
    header("Location:index.php?decryptedtext=".base64_encode($pass));
}
